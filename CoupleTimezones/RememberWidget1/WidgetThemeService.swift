...
class WidgetThemeService: NSObject {
    static let shared = WidgetThemeService()
    ...
    var theme: String {
        if let userTheme = WidgetHelpers.shared.getCurrentUser()?.theme {
            return userTheme
        }
        return "dark green"
    }
