...
class StateService: NSObject {
    ...
    func transferData() {
        if isConnected && isMatched {
            if canDownload == true && (canUpload == true || UserService.shared.get()?.canUpload == true) {
            ...
            if canDownload == true && (canUpload == false && UserService.shared.get()?.canUpload == false) {
                NotificationCenter.default.post(name: NSNotification.Name("ShouldDownload"), object: nil)
                AlarmClockService.shared.download()
            }
            if canDownload == false && (canUpload == true || UserService.shared.get()?.canUpload == true) {
